...
export function toRGBFromHex(hex: string): RGB {
...
export function toHSLFromHex(hex: string): HSL {
    const rgb = toRGBFromHex(hex);
    return toHSLFromRGB(rgb);
}

export function toHSLFromRGB(rgb: RGB): HSL {
    const { r, g, b } = rgb;
  ...
  export function toRGBFromHSL(hsl: HSL): RGB {
    let { h, s, l } = hsl;
